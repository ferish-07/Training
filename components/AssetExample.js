...
export default function AssetExample({ navigation }) {
  ...
  selectItem = (item) => {
    let temp = products.filter(
      (items) => items.category === item.category_name
    );

    navigation.navigate("SecondList", { name: temp });
    //  setData(temp)
  };
  selectedFilterText2 = (text) => {
    console.log("=====TEXT", text);
    if (text) {
      let newData = category.filter((item) => {
        let itemData = item.category_name.toUpperCase();
        // ? item.category_name.toUpperCase()
        // : ''.toUpperCase();
        let textData = text.toUpperCase();
        return itemData.indexOf(textData) > -1;
      });
      console.log("====adwadasdasdasdasd=====", newData);

      setFilter(newData);
    } else if (text.length == 0) {
      console.log("MEHUL BHAI");
      setFilter(category);
    }
  };
  return (
    <SafeAreaView style={{ backgroundColor: "white" }}>
      <View>
        <View
          style={{ width: "98%", alignItems: "center", flexDirection: "row" }}>
          <TextInput
            style={{
              borderWidth: 1,
              width: "90%",
            ...
            onChangeText={(text2) => selectedFilterText2(text2)}
          ...
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => selectItem(item)}>
              <View
                ...
                <Text style={{ margin: 20 }}>{item.category_name}</Text>
              </View>
            </TouchableOpacity>
          )}
        />
